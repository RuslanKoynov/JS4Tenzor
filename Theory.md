...
Можно использовать механизм "колбэков" (функции обратного вызова). Однако, если мы хотим загрузить несколько скриптов один за другим - это ведет к сложному, вложенному коду ("ад колбэков").

```javascript
getUserData(function(user) {
    getPosts(user.id, function(posts) {
        getComments(posts[0].id, function(comments) {
            console.log(comments); // Вложенность растет!
        });
    });
});
```
...
Выполни 1 и 2 [упражнения](practice.md)
...
### Обработка ошибок
Каждый promise предоставляет метод .catch() для перехвата ошибок. При этом `.catch()` перехватывает ошибки не только при `reject` promise, но и любые ошибки, возникшие в цепочке `.then()`
...
- `await` можно использовать только внутри async-функций. Он заставляет интерпретатор JavaScript ждать, пока promise справа от `await` не выполнится. Код приостанавливается, но не блокирует основной поток.
...
### Обработка ошибок
С `async/await` используется привычный синтаксис `try...catch`, что делает код более читаемым.
